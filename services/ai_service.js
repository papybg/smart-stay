...
function isHostVerified(authCode, userMessage) {
    ...
    console.log('[SECURITY] ❌ Верификация на домакина НЕУДАЧНА: не е намерено точно съответствие');
    return false;
}
...
🎟️ ТВОЯТА РЕЗЕРВАЦИЯ:
   ...
   Щифт на врата: ${data.lock_pin || '⏳ (ще бъде назначен при пристигане)'}${schedulerNote}
...
🔓 ⚠️ ЗАЩИТА НА ДОСТЪПА:
   ...
   За полен достъп: въведи твой HM код или направи нова резервация.
`;
    }
...
✅ ПРАВИЛО 1 - ИНФОРМАЦИЯ ЗА ИМОТА (STRICT MODE):
   • Въпроси за WiFi пароли, код на врата, правила, удобства → САМО от MANUAL
   • Въпроси за цени, резервации, политика на отмяна → САМО от MANUAL
   • Специфични детайли за имота (адрес, паркинг, басейн) → САМО от MANUAL
   • Контакти (телефон, имейл, име на домакин) → САМО от MANUAL
   ⚠️ ЗАБРАНЕНО: Измислени WiFi пароли, кодове или контакти!
...
export async function checkEmergencyPower(userMessage, role, bookingData) {
    ...
    // АВТОМАТИЗАЦИЯ НА ТОК: Само за гости
    if (role !== 'guest') {
        console.log('[POWER] Не е гост - пропускам проверките на ток');
        return "";
    }
    ...
    if (!needsPower && !needsMedical) {
        console.log('[POWER] Не са разпознати ключови думи за спешност');
        return "";
    }
    ...
    if (needsMedical) {
        ...
        // Автоматично вика спешна помощ (112)
        console.log('[POWER] 📞 АВТОМАТИЧНО ПОЗВАНИЕ: 112 спешна помощ');
        
        await automationClient.sendAlert(
            `🚑 КРИТИЧНО ЗДРАВОСЛОВНА СПЕШНОСТ: Гост ${bookingData?.guest_name} (${bookingData?.reservation_code}) докладва здравословна проблем. АВТОМАТИЧНО ПОЗВАНИЕ: Се препоръчва контакт със 112 или местната спешна помощ.`,
            {
        ...
        return "\n\n🚑 [СИСТЕМА СПЕШНОСТ: Голямо внимание! Системата е разпознала вероятна медицинска спешност. ПРЕПОРЪЧАНИЕ: Позвони 112 или местни спешни служби веднага! Домакинът е известен за вашата ситуация.]";
    }
    ...
    if (overrideSuccess) {
        console.log('[POWER] ✅ Команда за возстановяване на ток изпратена успешно към Telegram бот');
        
        // Изпраща известуване до домакина с информация за ситуацията
        await automationClient.sendAlert(
            `🚨 СПЕШНО ВЪЗСТАНОВЯВАНЕ НА ТОК: Гост ${bookingData?.guest_name} (${bookingData?.reservation_code}) докладва липса на ток. Ток е включен чрез Telegram команда (ВКЛ).`,
            {
                guest_name: bookingData?.guest_name || 'Непознат',
                reservation_code: bookingData?.reservation_code || 'N/A',
                role: role,
        ...
        return "\n\n✅ [СИСТЕМА: Системата за захранване е възстановена - електричеството трябва да бъде налично всеки момент]";
    } else {
        console.log('[POWER] ❌ Команда за управление на ток не успя');
        
        // Същинска грешка - уведомяваме домакина
        await automationClient.sendAlert(
            `🔴 КРИТИЧНО: Гост ${bookingData?.guest_name} докладва липса на ток, но АВТОМАТИЧНАТА КОМАНДА НА ТОК НЕ УСПЯ. Требует ручного вмешательства!`,
            {
                guest_name: bookingData?.guest_name || 'Непознат',
                reservation_code: bookingData?.reservation_code || 'N/A',
                role: role,
        ...
        return "\n\n⚠️ [СИСТЕМА: Възстановяване на ток е опитано, но е неудачно - контактувам домакина незабавно за приоритетна помощ]";
    }
}
