...
📍 АДРЕС: ул. Аспен Валей 1234, Планински Поглед
📞 КОНТАКТ: Достъпни 24/7 при спешност

🏊 УДОБСТВА:
- Външен басейн (сезонно)
- WiFi наличен
- Паркомясто достъпно

🔑 РЕЗЕРВАЦИЯ:
- За регистрирани гости: въведете вашия код HM...
...
const automationClient = {
     ...
     * КРИТИЧНО: Вика се само при спешни ситуации (гост докладва липса на ток)
     * @async
    ...
    async controlPower(state) {
        try {
            console.log('[AUTOMATION] Изпращам команда за управление на тока:', state ? 'ВКЛЮЧИ' : 'ИЗКЛЮЧИ');
            ...
            console.log('[AUTOMATION] Резултат на управлението на тока:', success ? 'УСПЕШНО' : 'НЕУДАЧНО');
            return success;
        } catch (e) {
...
// ============================================================================
// СТРОИТЕЛ НА СИСТЕМНО УКАЗАНИЕ - ОТГОВОРИ ВЪГЛЕД НА РОЛЯТА
// ============================================================================

/**
 * Строи приказ системно указание за AI въглед на ролята на потребителя
 * 
 * УКАЗАНИЕ НА ДОМАКИНА:
 * - Пълен достъп до наръчник
 * - Пълни способности за управление на ток
 * - Административни функции
 * 
 * УКАЗАНИЕ НА ГОСТ:
 * - Ограничен наръчник (информация, свързана само с резервация)
 * - Статус на ток само за четене
 * - Информация за щифт на врата
 * 
 * УКАЗАНИЕ НА НЕПОЗНАТ:
 * - Само PUBLIC_INFO_FALLBACK (не manual.txt)
 * - Никакви пароли, кодове или чувствителна информация
 * - Само информация за резервация
 * 
 * КРИТИЧНА ЗАЩИТНА СТЕНА: Непознатите НИКОГА не получават достъп до съдържанието на manual.txt
 * 
 * @param {string} role - Ролята на потребителя ('host', 'guest' или 'stranger')
 * @param {Object|null} bookingData - Информация за резервация на гост
 * @param {Object} powerStatus - Текущ статус на системата за тока
 * @param {string} manual - Съдържание на manual.txt (ако е налично)
 * @param {string} currentDateTime - Текущ низ с дата/час
 * @returns {string} Приказ системно указание за Gemini AI
 */
export function buildSystemInstruction(role, bookingData, powerStatus, manual, currentDateTime) {
    const { online, isOn } = powerStatus;

    console.log('[AI] Строя системно указание за ролята:', role);
    console.log('[AI] Статус на ток - онлайн:', online, '| включен:', isOn);

    if (role === 'host') {
        console.log('[AI] Строя указание за ДОМАКИН (пълен достъп)');
        return `
========================================================================
🏠 SMART-STAY AI АСИСТЕНТ - РЕЖИМ НА ДОМАКИН/АДМИНИСТРАТОР
========================================================================

📅 ТЕКУЩО ДАТА/ЧАС: ${currentDateTime} (българска часова зона)
🔐 НИВО НА ДОСТЪП: ПЪЛЕН АДМИНИСТРАТОР

⚡ СТАТУС НА СИСТЕМАТА ЗА ТОК:
   Мрежова връзка: ${online ? "✅ ОНЛАЙН" : "❌ ОФЛАЙН"}
   Захранване: ${isOn ? "✅ АКТИВНО" : "⚠️ НЕАКТИВНО"}

📋 ПЪЛЕН НАРЪЧНИК НА ИМОТА:
${manual}

🎯 ТВОИ СПОСОБНОСТИ:
   • Пълен достъп до вся информация на имота
   • Управление и мониторинг на системата за тока
   • Управление на гости и информация за резервации
   • Координиране на спешни известувания
   • Диагностика и отстраняване на системни проблеми

💬 ЕЗИ НА КОМУНИКАЦИЯ: Български (Български)
⚙️ ТОН: Професионален, всеобхватен, насочен към действие

ТИ СИ ЕКСПЕРТЕН УПРАВИТЕЛ НА ИМОТ. ПРЕДОСТАВЯЙ ПОДРОБНИ ТЕХНИЧЕСКИ ОТГОВОРИ.
========================================================================
`;
    }
...
👤 ТВОЯТА РЕЗЕРВАЦИЯ:
   Име на гост: ${bookingData.guest_name}
   Код на резервация: ${bookingData.reservation_code}
   Заселване: ${new Date(bookingData.check_in).toLocaleString('bg-BG')}
   Заминаване: ${new Date(bookingData.check_out).toLocaleString('bg-BG')}
   Код на врата: ${bookingData.lock_pin || '⏳ Назначава се...'}
` : '';

        console.log('[AI] Строя указание за ГОСТ (ограничен достъп)');
        return `
========================================================================
🏡 SMART-STAY AI АСИСТЕНТ - ДОБРЕ ДОШЛИ ГОСТ
========================================================================

📅 ТЕКУЩО ДАТА/ЧАС: ${currentDateTime} (българска часова зона)
🎯 ДОБРЕ ДОШЛИ В АПАРТАМЕНТ ASPEN VALLEY D105

${guestInfo}

📋 ИНФОРМАЦИЯ НА ИМОТА ЗА ТВОЯ ПРЕСТОЙ:
${manual}

⚡ СТАТУС НА СЪОРЪЖЕНИЯТА:
   Електричество: ${isOn ? "✅ Работи нормално" : "⚠️ Открит проблем - контактувам домакина"}
   Вода: ✅ Нормално
   WiFi: ✅ Налично (попросете парола, ако е необходимо)

🚨 АКО ИМАТЕ СПЕШНОСТ:
   • Контактувайте домакина веднага на предоставения номер
   • Използвайте [ALERT: описание] в съобщението си, ако е необходимо

💬 ЕЗИ НА КОМУНИКАЦИЯ: Български (Български)
⚙️ ТОН: Приятелски, полезен, отзивчив към нужди на гост

ТИ СИ ПРИВЕТЛИВИЯ АСИСТЕНТ НА ДОМАКИНА. БЕЙ ТОПЪЛ И ПОЛЕЗЕН ЗА ГОСТИТЕ.
========================================================================
`;
    }

    // УКАЗАНИЕ НА НЕПОЗНАТ - КРИТИЧНА ЗАЩИТНА СТЕНА
    console.log('[AI] Строя указание за НЕПОЗНАТ (само публична информация - БЕЗ manual.txt)');
    return `
========================================================================
🏠 SMART-STAY АСИСТЕНТ ЗА ПУБЛИЧНА ИНФОРМАЦИЯ
========================================================================

📅 ТЕКУЩО ДАТА/ЧАС: ${currentDateTime} (българска часова зона)
🔒 НИВО НА ДОСТЪП: САМО ПУБЛИЧНА ИНФОРМАЦИЯ

⚠️ ЗАБЕЛЕЖКА НА СИГУРНОСТ:
   Нямаш активна резервация. Защитена информация НЕ Е видима.

${PUBLIC_INFO_FALLBACK}

🔑 ЗА ОТКЛЮЧВАНЕ НА ФУНКЦИИ:
   • Съществуващи гости: Въведи твоя HM код на резервация, за да активираш пълни услуги
   • Нови посетители: Посетете нашия уебсайт, за да направите резервация

🚫 ОГРАНИЧЕНА ИНФОРМАЦИЯ (Не се споделя с незарегистрирани посетители):
   • Пароли на WiFi
   • Кодове за врата
   • Контактна информация на собственика
   • Вътрешни системи на имота
   • Информация за гост

💬 ЕЗИ НА КОМУНИКАЦИЯ: Български (Български)
⚙️ ТОН: Професионален, полезен, съзнателен относно сигурност

ТИ СИ АСИСТЕНТ НА ПУБЛИЧНА ИНФОРМАЦИЯ. ЗАЩИТАВАЙ ЧУВСТВИТЕЛНИ ДАННИ.
========================================================================
`;
}
...
export async function checkEmergencyPower(userMessage, role, bookingData) {
    ...
    // Разпознава ключови думи на български език, свързани с ток
    const powerKeywords = /няма ток|без ток|не работи ток|спрян ток|изключен ток|нямаэнергия/i;
    const needsPower = powerKeywords.test(userMessage);

    if (!needsPower) {
        console.log('[POWER] Не са разпознати ключови думи за спешност на ток');
        return "";
    }

    console.log('[POWER] ⚠️ СПЕШНОСТ РАЗПОЗНАТА: Гост докладва липса на ток!');

    // Получава текущия статус на тока
    const powerStatus = await automationClient.getPowerStatus();
    console.log('[POWER] Текущ статус на тока:', powerStatus);

    // Ако токът е действително изключен, задейства отговор на спешност
    if (powerStatus.online && !powerStatus.isOn) {
        console.log('[POWER] 🚨 ТОКЪТ Е ИЗКЛЮЧЕН - ЗАДЕЙСТВАМ АВТОМАТИЧНО ВЪЗСТАНОВЯВАНЕ');

        const success = await automationClient.controlPower(true);

        if (success) {
            console.log('[POWER] ✅ Команда за възстановяване на ток изпратена успешно');
            await automationClient.sendAlert(
                `🚨 СПЕШНО ВЪЗСТАНОВЯВАНЕ НА ТОК: Гост (${bookingData?.guest_name}) докладва липса на ток. Автоматично възстановяване на ток извършено.`,
                {
            ...
            return "\n\n✅ [СИСТЕМА: Спешно възстановяване на ток извършено - електричеството трябва да бъде възстановено всеки момент]";
        } else {
            console.log('[POWER] ❌ Команда за управление на ток не успя');
            return "\n\n⚠️ [СИСТЕМА: Възстановяване на ток е опитано, но е неудачно - контактувам домакина незабавно]";
        }
    }

    console.log('[POWER] Токът е онлайн и ВКЛЮЧЕН - не е необходимо действие при спешност');
    return "";
}
